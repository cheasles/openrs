    ...
    const int& kArchiveId, std::vector<uint8_t>* output)
{
    ...
    if (common::ifstream::GetStreamSize(*index_stream) < (kArchiveId + 1) * kIdxBlockLength)
    {
        return false;
    }
    ...
    while (remaining > 0)
    {
        ...
        const uint32_t kBlockSize = remaining > kBlockLength ? kBlockLength : remaining;
