...
public:
        ...
        const int& kSocketFileDescriptor)
    {
        ...
        if (0 == ::epoll_ctl(this->file_descriptor_, EPOLL_CTL_ADD, kSocketFileDescriptor, &event))
        {
            return true;
        }
    ...
    int Wait(std::vector<std::shared_ptr<Channel>>* channel_events)
    {
        ...
        const int kNumberEvents = ::epoll_wait(this->file_descriptor_, this->epoll_events_, MAX_EVENTS, 0);
        if (kNumberEvents <= 0)
        {
            return 0;
        }
