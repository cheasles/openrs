...
void openrs::net::io::BaseSocket::close()
{
    ...
    while(true)
    {
        ...
        switch(errno)
        {
            ...
            case EIO:   throw std::runtime_error("close: EIO");
            case EINTR:
            {
            ...
            default:    throw std::runtime_error("close: ???");
        }
    }
...
openrs::net::io::BaseSocket& openrs::net::io::BaseSocket::operator=(BaseSocket&& move) noexcept
{
    move.swap(*this);
    return *this;
}
...
std::size_t openrs::net::io::DataSocket::getMessageData(std::vector<uint8_t>* output)
{
    ...
    ::gettimeofday(&this->last_active_, nullptr);
    return data_read;
}

void openrs::net::io::DataSocket::putMessageData(const std::vector<uint8_t>& data)
{
    std::size_t     dataWritten = 0;

    while(dataWritten < data.size())
    {
        std::size_t put = write(getSocketId(), data.data() + dataWritten, data.size() - dataWritten);
