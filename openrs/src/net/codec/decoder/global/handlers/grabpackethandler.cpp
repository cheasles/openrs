    ...
    openrs::net::Client* client)
{
    ...
    uint32_t index_id;
    uint16_t* archive_id_ptr = nullptr;
    if (!packet.data.GetData(&priority_ptr) ||
        !packet.data.GetTriByte(&index_id) ||
    ...
    try
    {
        ...
        if (!store.GetArchiveData(index_id, ::be32toh(*archive_id_ptr), &cache_data))
        {
    ...
    cache_data_packet_buffer.PutData(static_cast<uint8_t>(index_id));
